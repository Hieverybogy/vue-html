<template>
    <div>
        ...
        <sapi-user
            ...
            :data.sync="data"
            :hasImg="vnode.attrs.hasImg"
            :type="vnode.attrs.type ? 'select-station' : 'select-user'"
            @callback="callback"
        ></sapi-user>
    </div>
</template>
...
import sapiUser from "@/components/selectUser/index.vue";
    ...
    methods: {
        format(item) {
            ...
            if (this.vnode.attrs.isCustomFormat) {
                ...
                str = `${this.vnode.attrs.format.indexOf('CorpName')>-1 ? item.company : ''}
                    ${this.vnode.attrs.format.indexOf('DeptName')>-1 ? '/' + item.department : ''}
                    ${item.name
                        ? "（" + item.name + "）"
                        : "（未关联员工）"}`
