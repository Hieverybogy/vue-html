...
ZmCanvasCrop.prototype = {
	...
	readFile: function () {
		...
		if (typeof FileReader === 'undefined') {
			...
			// self.errorCallBack(Vue.__t('userinfo.browsersDoNotSupport'));
			self.fileName = "";
		...
		function readFile() {
			...
			if (!/image\/\w+/.test(file.type)) {
				alert("文件必须为图片！");
				// self.errorCallBack(Vue.__t('userinfo.mustBeImage'));
				self.fileName = "";
				return false;
			}
			...
			if (!isCorrectFile) {
				alert("文件格式不正確");
				// self.errorCallBack(Vue.__t('userinfo.formatError'));
				self.fileName = "";
				return false;
			}
	...
	drawCavDown: function (src) {
		...
		self._img.onload = function () {
			...
			if (!self.imgTrue()) {
				...
				// self.errorCallBack(Vue.__t('userinfo.imageSize') + self._option.crop_min_width + '*' + self._option.crop_min_height);
				self.fileName = "";
				return;
			}
			//让宽或者高撑满
	...
	save: function () {
		if (!this.fileName) {
			alert("请先上传图片")
			// this.errorCallBack(Vue.__t('userinfo.imageInfo'));
			return;
		}
