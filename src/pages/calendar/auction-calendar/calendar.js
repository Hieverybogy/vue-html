...
var calendar = {
    ...
    */
    gregorianCalendar: function (Y, M, D) {
        ...
        for (i = 0; i < M - 1; i += 1) sum += calendar.solarMonth[i]
    ...
    */
    lunarCalendar: function (Y, M, D) {
        ...
        for (i = 0; i < M - 1; i++) sum += temp.lunarMonth[i] === 0 ? 29 : 30
...
function AuctionCalendar (options) {
...
AuctionCalendar.prototype = {
    _init () {
        if (utils.isString(this.date)) {
            this.date = utils.parseDate(this.date)
        }

        if (!utils.isDate(this.date)) {
            throw new Error('初始化日期格式不对')
        }

        this.now = new Date()
        this.todayYear = this.now.getFullYear()
        this.todayMonth = this.now.getMonth() + 1
        this.todayDate = this.now.getDate()

        this.year = this.date.getFullYear()
        this.month = this.date.getMonth() + 1
        this.selDate = this.date.getDate()
    ...
    initWeeksInfo () {
        ...
        // 当前月天数
        this.monthDaysCount = this.getMonthDaysCount(this.year, this.month)
        var temp = this.monthDaysCount + this.firstDay
        // 当前月显示周长度
        this.weeksLength = temp % 7 === 0 ? parseInt(temp / 7) : parseInt(temp / 7) + 1
    },
    generateWeeks () {
        const ws = []
        let prevMonthDaysCount
        let preMonthYear
        let preMonth
        let nextMonthYear
        let nextMonth
        this.todayData = null
        this.seleDayData = null

        this.initWeeksInfo()
        let week = []

        for (let i = 0; i < this.weeksLength * 7; i++) {
            if (i < this.firstDay) {
                if (!prevMonthDaysCount) {
                    preMonthYear = this.month === 1 ? this.year - 1 : this.year
                    preMonth = this.month === 1 ? 12 : this.month - 1
                    prevMonthDaysCount = this.getMonthDaysCount(preMonthYear, preMonth)
                }
                week.push(this.generateWeekDay(preMonthYear, preMonth, prevMonthDaysCount - (this.firstDay - 1 - i), 'prevMonth', ws.length))
            } else if (this.firstDay <= i && i < this.monthDaysCount + this.firstDay) {
                week.push(this.generateWeekDay(this.year, this.month, i - this.firstDay + 1, 'currMonth', ws.length))
            } else {
                if (!nextMonth) {
                    nextMonthYear = this.month === 12 ? this.year + 1 : this.year
                    nextMonth = this.month === 12 ? 1 : this.month + 1
                }

                week.push(this.generateWeekDay(nextMonthYear, nextMonth, i - (this.monthDaysCount + this.firstDay) + 1, 'nextMonth', ws.length))
            }
    ...
    generateWeekDay (year, month, date, type, weeksIndex) {
        const lunarInfo = this.getLunarInfo(year, month, date, type)
        const isToday = this.todayYear === year &&
            this.todayMonth === month && this.todayDate === date
        const isSeleDay = this.year === year &&
            this.month === month && this.selDate === date
        ...
        this.daysDD[`${year}-${month}-${date}`] = day
        ...
        if (isSeleDay) {
            this.seleDayData = day
            this.seleDayWeeksIndex = weeksIndex
        }
    ...
    // 阴历日期
    getLunarInfo (year, month, date) {
        if (calendar.lunarDateDD[`${year}-${month}-${date}`]) {
            return calendar.lunarDateDD[`${year}-${month}-${date}`]
        }
        var lunar = calendar.calendarConvert(year, month, date)
        var tm = month - 1
        // 本月对应节气的日期
        var tmp1 = this.sTerm(year, tm * 2) - 1
        var tmp2 = this.sTerm(year, tm * 2 + 1) - 1
        var solarTerms = ''
        var temp = ''
        // 对应的农历日期
        if (lunar[3] === 1) {
        ...
        // 判断今天时候是对应节气的日期
        if (tmp1 === date) {
            solarTerms = calendar.solarTerm[tm * 2]
        }
        if (tmp2 === date) {
            solarTerms = calendar.solarTerm[tm * 2 + 1]
        }
        // 显示  节日/节气/农历
        temp = calendar.lunarFestival[lunar[2] + '-' + lunar[3]] || solarTerms || temp

        calendar.lunarDateDD[`${year}-${month}-${date}`] = {
            text: temp,
            solarTerm: !!solarTerms,
            lunarFestival: !!calendar.lunarFestival[lunar[2] + '-' + lunar[3]]
        }
    ...
    prevMonth () {
        ...
        this.selDate = 1
        this.date = new Date(`${this.year}/${this.month}/${this.selDate}`)
    ...
    updateSeleDate (date) {
        ...
        if (!utils.isDate(this.date)) {
            throw new Error('updateSeleDate日期格式不对')
        }
        ...
        if (this.date.getFullYear() !== this.year ||
            ...
            this.selDate = this.date.getDate()
            if (this.seleDayData) {
                this.seleDayData.seleDay = false
            }

            this.seleDayData = this.daysDD[`${this.year}-${this.month}-${this.selDate}`]
            this.seleDayData.seleDay = true
        }
